...
import type { NotesPatchRequest } from '../../apps/hosted/src/types/api';
...
export const handler: Handler = async (event) => {
  ...
  if (event.httpMethod === 'PATCH') {
    try {
      ...
      if (!parsed.success) {
        console.warn('Notes save: invalid payload', parsed.error.flatten());
      ...
      if (written.errors.length > 0) {
        console.warn('Notes save: stored config fails validation, persisting anyway', {
          publicDisplayId,
